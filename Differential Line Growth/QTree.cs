...
namespace Differential_Line_Growth
{
    class QTree
    {
        ...
        private List<object> my_Objects;
        ...
        public bool Insert(Vector pt, object obj)
        {
            bool result;
            if (pt.X < my_Boundary.Left || pt.X > my_Boundary.Right || pt.Y < my_Boundary.Top || pt.Y > my_Boundary.Bottom)
            {
            ...
            else
            {
                ...
                result = my_Branches[0].Insert(pt, obj);
                if (!result) result = my_Branches[1].Insert(pt, obj);
                if (!result) result = my_Branches[2].Insert(pt, obj);
                if (!result) result = my_Branches[3].Insert(pt, obj);
            }
            return result;
        }
        ...
        public List<T> QueryObjects<T>(Rect area)
        {
            ...
            else
            {
                for (int i = 0; i < my_Points.Count; i++)
                {
                    if (area.Contains((Point)my_Points[i])) ;
                    {
                        result.Add((T)my_Objects[i]);
                    }
                }
                if (divided)
                {
                    result.AddRange(my_Branches[0].QueryObjects<T>(area));
                    result.AddRange(my_Branches[1].QueryObjects<T>(area));
                    result.AddRange(my_Branches[2].QueryObjects<T>(area));
                    result.AddRange(my_Branches[3].QueryObjects<T>(area));
                }
            }
            return result;
        }

        public List<T> QueryObjects<T>(Vector center, double distance)
        {
            List<T> result = new List<T>();
            if (!my_Boundary.IntersectsWith(new Rect(center.X - distance, center.Y - distance, 2 * distance, 2 * distance)))
            {
            ...
            else
            {
                ...
                if (divided)
                {
                    result.AddRange(my_Branches[0].QueryObjects<T>(center, distance));
                    result.AddRange(my_Branches[1].QueryObjects<T>(center, distance));
                    result.AddRange(my_Branches[2].QueryObjects<T>(center, distance));
                    result.AddRange(my_Branches[3].QueryObjects<T>(center, distance));
                }
            }
            return result;
        }

...
namespace Fourier_Complex
{
    class DFT
    {
        ...
        public static List<double> Process2double(List<double> signal)
        {
            int N = signal.Count;
            ...
            for (int n = 0; n < N; n++)
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    alfa = 2 * Math.PI * n * k / N;
                    Re += signal[k] * Math.Cos(alfa);
                    Im += -1 * signal[k] * Math.Sin(alfa);
                }
                //result.Add((Re * Re + Im * Im) / (N * N));
                result.Add((Math.Abs(Re) + Math.Abs(Im)) / N);
            }
            return result;
        }

        public static double[] Process2double(double[] signal)
        {
            int N = signal.Length;
            ...
            for (int n = 0; n < N; n++)
            {
                ...
                result[n] = (Re * Re + Im * Im) / (N * N);
                //result[n] = (Math.Abs(Re) + Math.Abs(Im)) / N;
            }
            return result;
        }

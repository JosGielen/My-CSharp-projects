...
const int BARS = 2;
const int BALL = 3;
const vec4 BASE_MAT = vec4(0.35, 0.25, 0.0, 0.8);  //brown
const vec4 BARS_MAT = vec4(0.5, 0.5, 0.5, 0.8);    //grey
...
vec3 Render(vec3 ro, vec3 rd)
{
    ...
    if (t.y == BARS)
    {
        col += compute_reflection(p, n, ro, 0.5 * mat.w);
        return col;
    }
    //Add the reflections for the base and the balls
    for (int i = 0; i < 1; i++)
    {
        ro = p + n * 3 * SURF_DIST;
        rd = r;
